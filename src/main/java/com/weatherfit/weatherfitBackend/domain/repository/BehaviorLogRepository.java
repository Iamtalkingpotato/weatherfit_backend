...
public interface BehaviorLogRepository extends JpaRepository<BehaviorLog, Long> {
    ...
    // ── 1. 요약 (summary) ─────────────────────────────────────────────────────
    @Query("SELECT COUNT(b) FROM BehaviorLog b WHERE b.eventType IN ('page_view', 'VIEW') AND (:since IS NULL OR b.createdAt >= :since)")
    long countViews(@Param("since") LocalDateTime since);

    @Query("SELECT AVG(b.duration) FROM BehaviorLog b WHERE b.duration IS NOT NULL AND (:since IS NULL OR b.createdAt >= :since)")
    Double avgDuration(@Param("since") LocalDateTime since);

    @Query("SELECT AVG(b.scrollDepth) FROM BehaviorLog b WHERE b.scrollDepth IS NOT NULL AND (:since IS NULL OR b.createdAt >= :since)")
    Double avgScrollDepth(@Param("since") LocalDateTime since);
    ...
    @Query(value = "SELECT HOUR(created_at) AS hour, COUNT(*) AS cnt " +
                   "FROM behavior_log " +
                   "WHERE created_at IS NOT NULL AND (:since IS NULL OR created_at >= :since) " +
    ...
    @Query(value = "SELECT COUNT(DISTINCT CONCAT(" +
                   ...
                   "AND (:since IS NULL OR created_at >= :since)",
           nativeQuery = true)
    long countPurchases(@Param("since") LocalDateTime since);

    @Query("SELECT COUNT(b) FROM BehaviorLog b WHERE b.eventType IN ('product_view', 'PRODUCT_VIEW') AND (:since IS NULL OR b.createdAt >= :since)")
    long countProductViews(@Param("since") LocalDateTime since);
    ...
    @Query(value = "SELECT item_id, COUNT(*) AS cnt FROM behavior_log " +
           ...
           nativeQuery = true)
    List<Object[]> findTopProductsByView(@Param("since") LocalDateTime since);
    ...
    @Query(value = "SELECT page_url, COUNT(*) AS cnt FROM behavior_log " +
           ...
           nativeQuery = true)
    List<Object[]> findTopPagesByView(@Param("since") LocalDateTime since);
    ...
    @Query(value = "SELECT COUNT(DISTINCT CONCAT(" +
                   ...
                   "AND (:since IS NULL OR created_at >= :since)",
           nativeQuery = true)
    long countWishlists(@Param("since") LocalDateTime since);
    ...
    @Query(value = "SELECT COUNT(DISTINCT CONCAT(" +
                   ...
                   ")",
           nativeQuery = true)
    long countWishlistConversions(@Param("since") LocalDateTime since);
}
